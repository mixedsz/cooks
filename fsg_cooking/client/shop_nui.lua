...
local function canAfford(amount, method)
    ...
    elseif method == 'bank' then
        return lib.callback.await('fsg_cooking:checkBankBalance', false, amount)
    end
    return false
end
...
local function deductPayment(amount, method)
    ...
    elseif method == 'bank' then
        return lib.callback.await('fsg_cooking:removeBankMoney', false, amount)
    end
    return false
end
...
Nui:cb('purchaseItems', function(data, cb)
    ...
    if paymentMethod == 'cash' then
        local hasFunds = lib.callback.await('fsg_cooking:checkCashBalance', false, total)
    ...
    if paymentMethod == 'bank' then
        local hasFunds = lib.callback.await('fsg_cooking:checkBankBalance', false, total)
        if not hasFunds then
            cb({ success = false, message = 'Insufficient funds' })
            return
        end
    ...
    local success, message = lib.callback.await('fsg_cooking:purchaseItems', false, items, paymentMethod)
