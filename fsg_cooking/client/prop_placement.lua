-- ============================================================
-- fsg_cooking | client/prop_placement.lua
...
function loadModel(hash)
    if Config.Debug then
        print("^3[fsg_cooking]^7: Loading model: " .. tostring(hash))
    end
    ...
    if not HasModelLoaded(hash) then
        ...
        if not HasModelLoaded(hash) then
            print("^1[fsg_cooking]^7: Failed to load model: " .. tostring(hash))
            return false
        end
    end
    return true
...
--    callback fsg_cooking:server:placeProp.
function StartPropPlacement(itemName)
    ...
    if Config.Debug then
        print("^3[fsg_cooking]^7: Setting placementLock to true")
    end
    placementLock = true
    ...
    if not foundPropData then
        if Config.Debug then
            print("^3[fsg_cooking]^7: Clearing placementLock due to invalid prop type")
        end
        placementLock = false
        return
    end
    ...
    if gizmoData then
        ...
        if Config.Debug then
            print("^3[fsg_cooking]^7: Gizmo data received:", json.encode(gizmoData))
            print("^3[fsg_cooking]^7: Actual object coords:", finalCoords)
            print("^3[fsg_cooking]^7: Actual object rotation:", finalRotation)
        end
        ...
        if coStatus and coResult then
            -- ─── Placement confirmed ─────────────────────────────
            if Config.Debug then
                print("^3[fsg_cooking]^7: Gizmo placement confirmed, deleting gizmo object")
            end
            ...
            if Config.Debug then
                print("^3[fsg_cooking]^7: Calling server to place prop")
            end
            ...
            if not IsPlayerInjured() then
                ...
                if Config.Debug then
                    ...
                    print("^3[fsg_cooking]^7: Server responded with: " .. tostring(serverResult))
                    print("^3[fsg_cooking]^7: Clearing placementLock after server response")
                end
        ...
        if Config.Debug then
            print("^3[fsg_cooking]^7: Gizmo placement cancelled (no data returned from gizmo)")
        end
        DeleteEntity(gizmoEntity)
    end
...
function ClearPlacementLock()
    if Config.Debug then
        print("^3[fsg_cooking]^7: Force clearing placement lock")
    end
    placementLock = false
    lib.callback.await("fsg_cooking:server:clearActivePlacement", false)
end
...
CreateThread(function()
    ...
    while true do
        ...
        if placementLock and not isPlacing then
            if Config.Debug then
                print("^3[fsg_cooking]^7: Detected stale placement lock, clearing it")
            end
            ClearPlacementLock()
        end
        Wait(1000)
...
lib.callback.register("fsg_cooking:client:syncProp", function(netId, model, coords, rotation, propId)
...
lib.callback.register("fsg_cooking:client:createPropLocally", function(model, coords, rotation, owner, propId)
    ...
    if not owner then
        ...
        print("^3[fsg_cooking]^7: Using fallback owner value:", owner)
    end

    -- Fallback propId
    if not propId then
        propId = "local_" .. model .. "_" .. math.random(1000, 9999) .. "_" .. GetGameTimer()
        print("^3[fsg_cooking]^7: Using fallback propId:", propId)
    end
    ...
    if not loadModel(modelHash) then
        print("^1[fsg_cooking]^7: Failed to load model for prop creation")
        return false
    end
...
local function createViewOnlyProp(model, coords, rotation, owner, propId)
    if Config.Debug then
        print("^3[fsg_cooking]^7: Received createViewOnlyProp event with parameters:")
    ...
    if not owner then
        owner = 0
        if Config.Debug then
            print("^3[fsg_cooking]^7: Using fallback owner value for view-only prop:", owner)
        end
    end
    ...
    if not propId then
        ...
        if Config.Debug then
            print("^3[fsg_cooking]^7: Using fallback propId for view-only prop:", propId)
        end
    end
    ...
    local modelHash = joaat(model)
    if not loadModel(modelHash) then
        print("^1[fsg_cooking]^7: Failed to load model for view-only prop creation")
        return false
    end
...
lib.callback.register("fsg_cooking:client:createViewOnlyProp", function(model, coords, rotation, owner, propId)
    return createViewOnlyProp(model, coords, rotation, owner, propId)
end)

-- ─── RegisterNetEvent: createViewOnlyProp ───────────────────
RegisterNetEvent("fsg_cooking:client:createViewOnlyProp")
AddEventHandler("fsg_cooking:client:createViewOnlyProp", function(model, coords, rotation, owner, propId)
...
lib.callback.register("fsg_cooking:client:deletePropById", function(propId)
...
RegisterNetEvent("fsg_cooking:client:deletePropById")
AddEventHandler("fsg_cooking:client:deletePropById", function(propId)
...
lib.callback.register("fsg_cooking:client:deleteEntityByNetId", function(netId)
...
RegisterNetEvent("fsg_cooking:client:deleteEntityByNetId")
AddEventHandler("fsg_cooking:client:deleteEntityByNetId", function(netId)
...
RegisterNetEvent("fsg_cooking:client:registerTargetForModel")
AddEventHandler("fsg_cooking:client:registerTargetForModel", function(model)
