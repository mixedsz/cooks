...
local function removeBankMoney(source, amount)
    ...
    elseif framework == 'qb' then
        local Player = QBCore.Functions.GetPlayer(source)
        if Player then
            Player.Functions.RemoveMoney('bank', amount, 'fsg_cooking_purchase')
            return true
        end
    end
    return false
...
local function loadPropsFromDB(cb)
    MySQL.Async.fetchAll('SELECT * FROM fsg_cooking_props', {}, function(results)
        if not results then
            if cb then cb() end
            return
        end
        ...
        if Config.Debug then
            print('^3[fsg_cooking]^7: Loaded ' .. #results .. ' persistent props from DB')
        end
        if cb then cb() end
    end)
end
...
RegisterNetEvent('fsg_cooking:server:requestProps', function()
    local src = source
...
lib.callback.register('fsg_cooking:server:checkRequiredItems', function(source, data)
...
lib.callback.register('fsg_cooking:server:completeCooking', function(source, data)
    if not data or not data.recipe then return false end
    local recipe = Config.Recipes[data.recipe]
    if not recipe or not recipe.requiredItems then return false end
...
lib.callback.register('fsg_cooking:getShopData', function(source, storeIndex)
...
lib.callback.register('fsg_cooking:checkCashBalance', function(source, amount)
...
lib.callback.register('fsg_cooking:checkBankBalance', function(source, amount)
    if not amount or amount <= 0 then return true end
    return getBankBalance(source) >= amount
end)

-- Remove money from player bank
lib.callback.register('fsg_cooking:removeBankMoney', function(source, amount)
...
lib.callback.register('fsg_cooking:purchaseItems', function(source, items, paymentMethod)
...
lib.callback.register('fsg_cooking:server:placeProp', function(source, model, coords, rotation)
    ...
    if Config.PersistentProps.Enabled then
        MySQL.Async.execute(
            'INSERT INTO fsg_cooking_props (prop_id, model, x, y, z, rot_x, rot_y, rot_z, owner) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    ...
    if Config.Debug then
        print('^3[fsg_cooking]^7: Prop ' .. propId .. ' placed by ' .. identifier)
    end
...
lib.callback.register('fsg_cooking:server:pickupProp', function(source, netId, propId)
    ...
    if not prop and Config.PersistentProps.Enabled then
        local results = MySQL.Sync.fetchAll('SELECT * FROM fsg_cooking_props WHERE prop_id = ?', { propId })
    ...
    if Config.PersistentProps.Enabled then
        MySQL.Async.execute('DELETE FROM fsg_cooking_props WHERE prop_id = ?', { propId })
    end
...
lib.callback.register('fsg_cooking:server:clearActivePlacement', function(source)
    ...
    CreateThread(function()
        while true do
            ...
            if Config.Debug then
                ...
                print('^3[fsg_cooking]^7: ' .. count .. ' props currently tracked in memory')
            end
        end
    end)
end
